...
- **`cloneBytes` helper**: package-local function duplicated in `bip/frame.go` and `apdu/types.go`; use it for all defensive copies of `[]byte` fields — do not introduce a shared package for it.
- **Constants**: `BACnetIpDefaultUdpPort uint16 = 0xbac0` in `bip/constants.go`; `DefaultMaxDatagramSize = 1476` in `bip/transport.go` (conservative IPv4 UDP payload budget). Root package `constants.go` also exposes `DefaultPort uint16 = 0xBAC0` (same value, different symbol).
