...
func TestNewClientRuntimeWithConn(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil conn", conn: nil, cfg: ClientRuntimeConfig{ASE: aseCfg}, wantErr: ErrNilDatagramConn},
...
func TestClientRuntimeRunWritePropertyRoundTrip(t *testing.T) {
	...
	conn.writeHook = func(data []byte, addr netip.AddrPort) {
		...
		ackFrame, err := NewOriginalUnicastNpdu(BVLCTypeBACnetIP, ackNPDUBytes)
		if err != nil {
	...
	dst, err := AddrPortToAddress(peer)
	if err != nil {
